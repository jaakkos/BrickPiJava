...
public class BrickPi extends BrickPiCommunications {
     ...
     * @param packet
     */
    protected void sendToBrickPi(byte destinationAddress, byte[] packet) throws IOException {
    ...
    protected byte[] readFromBrickPi(int timeout) throws IOException { // timeout in mS
        ...
        while (serial.available() < 2 && delay-- >= 0) { // we need at least the checksum and bytecount (2 bytes)
            LOGGER.debug("Available: {}", serial.available());
            try {
                Thread.sleep(5);  // 5ms
        ...
        byte[] allbytes = serial.read();
        byte checksum = allbytes[0];
        byte packetSize = allbytes[1];  // the packet size does not include this two byte header.
        ...
        while (serial.available() < packetSize && delay-- >= 0) { // we need at least the checksum and bytecount (2 bytes)
            try {
                Thread.sleep(5);  // 5ms
        ...
        for (int counter = 0; counter < packetSize; counter++) {
            packet[counter] = (byte) (allbytes[counter + 2] & 0xFF);
